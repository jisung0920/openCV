...
String test ="D:\\backup\\hanyang_17_01\\멀티미디어정보처리\\실습자료\\hw2\\10.jpg";//5,7
int winNum;
int saveNum=0;
...
{
    winNum = 1;
    Mat img = imread(GAUSI,CV_LOAD_IMAGE_COLOR);
    Mat img2 = imread(test,CV_LOAD_IMAGE_COLOR);
    Mat img3 = imread(test,CV_LOAD_IMAGE_COLOR);
    Mat img4 = imread(IMPUL3,CV_LOAD_IMAGE_GRAYSCALE);
    m_show(img2,SAVE);
  //  m_Blur(img2,1.0/27.0);

    //m_Blur_5(img2,1.0/5.0);
    m_Shar(img2,0,-1,5);
    m_Shar(img2,-1,-1,11);
    //m_Nois_M(img4);
